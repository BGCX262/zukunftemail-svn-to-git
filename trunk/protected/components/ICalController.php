...
class ICalController extends CController
{
    ...
    $dates = EDateiCal::date_repeat_calc($rule, $start, $end, array());
    ...
    $result = implode(', ', $dates);
    print_r($result); //($result, $shouldbe, $rule .'; Starting '.$start .';  results: '. $result);
    ...
    //      (September 3, 1997 EDT)9:00,9:20,9:40,10:00,10:20,16:00,16:20,16:40
 	...
 	function actionInput() {
 		...
 		if (isset($_POST['RRule'])){
 			foreach ($_POST['RRule'] as $k => $v) {
 				$model->$k = $v;
 			}
 		}
 		$this->render("rruleform", array('model' => $model));
 	}

 	function actionTestinput() {
 		$form_values = array(
 			'RRULE' => array (
	        	'FREQ' => array ('MONTHLY'),
		        'BYDAY' => array ('1SU', '-1SU'),
		...
		print "<h3>OUTPUT RULE</h3>";
		print "<pre>";
		print_r (EDateiCal::date_api_ical_build_rrule($form_values));
		print "</pre>";
 	}

 	function actionParse() {
 		$file = Yii::getPathOfAlias('webroot.files') . DIRECTORY_SEPARATOR . 'USHolidays.ics' ;
 		print "<pre>";
 		var_export(EDateiCal::date_ical_import($file));
 		print "</pre>";
 	}

 	function actionFullCalendar() {
 		$file = Yii::getPathOfAlias('webroot.files') . DIRECTORY_SEPARATOR . 'USHolidays.ics' ;
 		$data = EDateiCal::date_ical_import($file);
 		$this->render('fullCalendar', array('events' => $data));
 	}
